...
test.describe('Simple Web Interface Test', () => {
  test('should load page and check JavaScript functionality', async ({ page }) => {
    ...
    // Wait a bit to capture any initial console errors
  ...
  test('should handle form interaction without file upload', async ({ page }) => {
    await page.goto('http://localhost:8080');
  ...
  test('should check network requests', async ({ page }) => {
    ...
    expect(requests.some(req => req.includes('main.js'))).toBe(true);
    expect(requests.some(req => req.includes('bootstrap'))).toBe(true);
  });
});