...
test.describe('Server API Endpoints', () => {
  ...
  test('should convert Word document via POST /raw endpoint', async ({ request }) => {
  ...
  test('should convert multiple headings document via API', async ({ request }) => {
    const fixturePath = path.join(__dirname, '../../__fixtures__/multiple-headings.docx');
    const fileBuffer = fs.readFileSync(fixturePath);
  ...
  test('should handle text formatting via API', async ({ request }) => {
    const strongFixturePath = path.join(__dirname, '../../__fixtures__/strong.docx');
  ...
  test('should reject .doc files with proper error message', async ({ request }) => {
    ...
    expect(errorMessage).toContain('This tool only supports .docx files, not .doc files');
  });
  ...
  test('should handle files with spaces in names', async ({ request }) => {
    const fixturePath = path.join(__dirname, '../../__fixtures__/file with space.docx');
    const fileBuffer = fs.readFileSync(fixturePath);
  ...
  test('should handle complex document with various elements', async ({ request }) => {
    const fixturePath = path.join(__dirname, '../../__fixtures__/list-with-links.docx');
    const fileBuffer = fs.readFileSync(fixturePath);
  ...
  test('should validate API response content-type and structure', async ({ request }) => {
    ...
    expect(contentType).toContain('text/html'); // Express default content-type
