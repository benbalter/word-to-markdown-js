...
test.describe('Word to Markdown Web Interface', () => {
  ...
  test('should upload and convert a simple Word document', async ({ page }) => {
    ...
    await expect(page.locator('#output')).not.toHaveText('', { timeout: 10000 });
  ...
  test('should show error for unsupported .doc files', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      const file = new File(['fake content'], 'document.doc', { type: 'application/msword' });
      dt.items.add(file);
    ...
    // Note: The exact error display mechanism depends on the implementation
    // We might need to check for error styling or message elements
    await page.waitForTimeout(1000); // Give time for error to appear
    
    // Results should still be hidden if error occurred
    await expect(page.locator('#results')).toHaveClass(/d-none/);
  });
  ...
  test('should have working navigation links', async ({ page }) => {
    ...
    await expect(page.locator('a[href="https://github.com/benbalter/word-to-markdown-js"]')).toBeVisible();
    await expect(page.locator('a[href*="patreon.com/benbalter"]')).toBeVisible();
